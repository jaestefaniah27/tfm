...
entity tb_rx_top_real is
end tb_rx_top_real;

architecture sim of tb_rx_top_real is
  ...
  constant WR_GAP  : integer := 8681;  -- 1 byte a 115200 baudios @100MHz
  ...
  -- Latencia de rearmado del BD del MCDMA tras cada TLAST (ciclos de 100 MHz)
  constant BD_REARM_CYCLES : integer := 4000;   -- 40 us, orden de una ISR
...
begin
  ...
  -- ── Modelo de la contrapresión del MCDMA ────────────────────────────────
  -- Tras cerrar un paquete, el MCDMA no acepta más hasta que la ISR rearma el
  -- descriptor. Mientras tanto TREADY baja y el DRAIN se queda bloqueado.
  bp_proc : process
  begin
    m_tready <= '1';
    wait until rising_edge(clk) and m_tvalid = '1' and m_tlast = '1' and m_tready = '1';
    m_tready <= '0';
    for i in 0 to BD_REARM_CYCLES - 1 loop
      wait until rising_edge(clk);
    end loop;
  end process;
