...
entity CONFIGURABLE_SERIAL is
  ...
  generic (
    DELAY_CYCLES : integer := 10000   -- settling de DE (ver TX_CONFIGURABLE_SERIAL)
  );
  port (
    ...
    -- DEBUG (diagnóstico de simulación): {EMPTY, Fifo_write, Valid_out, LineRD_in}
...
architecture RTL of CONFIGURABLE_SERIAL is
 ...
 ------------------------------------------------------------------------
  ...
  signal empty_i    : std_logic;  -- copia interna del 'empty' del FIFO (para dbg_rx)
  signal fifo_rd    : std_logic;  -- rd_en del FIFO = beat AXI-Stream (no flanco)
...
begin  -- RTL
  ...
  -- Lectura del FIFO sincronizada con el beat AXI-Stream:
  --   rd_en = (FIFO no vacío) AND (Data_read = consumidor listo)
  -- Data_read lo conduce el wrapper directamente con m_axis_rx_tready (nivel),
  -- de modo que el FIFO avanza EXACTAMENTE en el ciclo en que el DMA acepta el
  -- byte. Antes se usaba el flanco de Data_read (data_read_flanc), cuya latencia
  -- (registro + detector de 2 etapas) hacía que el DMA releyera el mismo dato
  -- del FIFO FWFT varias veces → bytes triplicados.
  fifo_rd <= (not empty_i) and Data_read;
 ...
 --   if Valid_D = '0' and TX_RDY_i = '1' then
  ...
  -- DEBUG: señales internas del RX para diagnóstico en simulación
  dbg_rx <= empty_i & Fifo_write & Valid_out & LineRD_in;
