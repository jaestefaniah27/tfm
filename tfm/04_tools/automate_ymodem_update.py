...
def try_jtag_reset():
    """Intenta reiniciar la placa programáticamente a través de JTAG usando Xilinx XSDB."""
    xsdb_path = "/tools/Xilinx/2025.1/Vivado/bin/xsdb"
    if not Path(xsdb_path).exists():
        xsdb_path = "/tools/Xilinx/2025.1/Vitis/bin/xsdb"
        if not Path(xsdb_path).exists():
            print("[i] XSDB JTAG debugger no encontrado en rutas estándar. Omitiendo reset automático.")
            return False

    print("[i] Intentando resetear la placa automáticamente por JTAG (XSDB)...")
    
    tcl_content = """connect
set targets_list [targets]
if {$targets_list eq ""} {
  puts "ERROR_NO_TARGETS"
} else {
  if {[catch {targets -set -nocase -filter {name =~ "*PSU*"}; rst -system} err]} {
    if {[catch {targets -set -nocase -filter {name =~ "*DAP*"}; rst -system} err]} {
      if {[catch {targets -set -nocase -filter {name =~ "*Cortex-A53*"}; rst -system} err2]} {
         puts "ERROR_RESET_FAILED"
      } else {
         puts "RESET_OK"
      }
    } else {
...
"""
    ...
    temp_tcl = Path("/tmp/reset_zcu102.tcl")
    try:
        temp_tcl.write_text(tcl_content)
        res = subprocess.run(
            [xsdb_path, str(temp_tcl)],
        ...
        try:
            temp_tcl.unlink() # Limpiar archivo temporal
        except Exception:
            pass
            
        if "RESET_OK" in res.stdout:
            print("[OK] ¡Placa reseteada con éxito por JTAG!")
            return True
        elif "ERROR_NO_TARGETS" in res.stdout:
            print("[w] No se encontraron dispositivos JTAG conectados localmente.")
            print("[i] CONSEJO: Conecta el cable micro-USB al puerto JTAG (J2) de la ZCU102 y pásalo a esta máquina virtual en tu hipervisor para automatizar el reset al 100%.")
        else:
            print(f"[w] Error al resetear por JTAG: {res.stdout.strip()}")
            if res.stderr:
                print(f"[w] Detalle de error (stderr): {res.stderr.strip()}")
    except Exception as e:
        print(f"[w] No se pudo ejecutar el reset por JTAG: {e}")
...
def main():
    ...
    # Intentar reset automático por JTAG
    jtag_success = try_jtag_reset()
    if not jtag_success:
        print("[i] Por favor, pulsa el botón de RESET físico (POR_B o SRST_B) en la placa ZCU102 para iniciar el arranque...")
