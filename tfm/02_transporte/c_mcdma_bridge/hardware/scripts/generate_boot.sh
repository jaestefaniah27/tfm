...
VITIS_WS="${VITIS_DIR}/vitis_ws"   # workspace dedicado; se borra/recrea en cada run
VIVADO_DIR="/home/mpsocv2/quick-start/app/serial_bridge/hardware/"
...
ask_config_name() {
    ...
    FSBL_ELF="${VITIS_WS}/${FSBL_NAME}/build/${FSBL_NAME}.elf"
    XPFM_PATH="${VITIS_WS}/${PLATFORM_NAME}/export/${PLATFORM_NAME}/${PLATFORM_NAME}.xpfm"
...
run_vitis_setup() {
    ...
    # Verificar si ya existe un workspace previo
    if [[ -d "${VITIS_WS}" ]]; then
        log_warn "Ya existe workspace en '${VITIS_WS}'."
        if prompt_confirm "¿Eliminar y recrear?"; then
            log_info "Eliminando workspace anterior..."
            rm -rf "${VITIS_WS:?}"
            log_ok "Workspace eliminado"
        else
            log_info "Manteniendo workspace existente."
            if [[ -f "${FSBL_ELF}" ]]; then
    ...
    # Crear workspace limpio — sin .metadata previo, sin conflicto de versión
    mkdir -p "${VITIS_WS}"
    log_info "Workspace Vitis limpio: ${VITIS_WS}"
...
# Eliminar .metadata que create_client() pudo haber creado antes de set_workspace
_meta = os.path.join("${VITIS_WS}", ".metadata")
...
PYEOF
    ...
    if "${VITIS_BIN}" -s "${vitis_script}" 2>&1; then
        ...
        if [[ -f "${FSBL_ELF}" ]]; then
            ...
            local found_elf
            found_elf=$(find "${VITIS_WS}/${FSBL_NAME}" -name "*.elf" 2>/dev/null | head -1)
            if [[ -n "${found_elf}" ]]; then
                FSBL_ELF="${found_elf}"
