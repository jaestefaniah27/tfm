...
/**
 * @brief Envía una cadena (no bloqueante: encola y arranca la TX por IRQ).
 */
int Transceiver_SendString(Transceiver *dev, const char *s);

/**
 * @brief Envío binario por longitud (admite bytes 0x00, para frames de protocolo).
 * @return Número de bytes encolados, o -1 si el buffer de TX estaba lleno.
 */
...
/* Instrumentación común a las tres variantes. Esta variante no define
 * MCDMA_BASE ni DMA0_BASE, por lo que se autodetecta como GPIO. */
