...
# Versión 7.2: Generador Autónomo con SysInfo Enriquecido y Puertos Explícitos
# -----------------------------------------------------------------------------------------
# Características:
...
# 5. Hardware Ports: Mapeo explícito de RX, TX, DE (FSM) y SLO (GPIO) para todos los canales.
# -----------------------------------------------------------------------------------------
...
proc create_transceiver_hier { parent_name index } {
  ...
  # Ch1 (Out): Configuración + TX Data + SLO | Ch2 (In): Status + RX Data
  ...
  # 6. Pines UART (Físicos) - DE sale de FSM, SLO sale de registro interno alimentado por GPIO
...
proc create_many_transceivers { count ps_name main_xbar_name } {
  puts "------------------------------------------------"
  puts " INICIANDO GENERACIÓN INTELIGENTE V7.1 ($count CANALES)"
  puts "------------------------------------------------"
  ...
  for {set i 0} {$i < $count} {incr i} {
    ...
    # Creación explícita de puertos para evitar bugs de renombrado automático en Vivado
...
#source /home/mpsocv2/quick-start/app/configurable_transceiver_inter/scripts/new_generate_transceivers.tcl

#create_many_transceivers 4 "zynq_ultra_ps_e_0" "axi_smc"
