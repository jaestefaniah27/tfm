...
organizados en tres bloques según la placa validada: la sección~\ref{sec:validacion_cdhs} cubre
las pruebas realizadas con la placa CDHS, la sección~\ref{sec:validacion_aocs} las realizadas con
la placa AOCS, y la sección~\ref{sec:validacion_propia} recoge la validación de la placa de
comunicación serie: la comparativa de las tres arquitecturas de transporte
PS--PL y la caracterización eléctrica de la placa ya fabricada, sobre sus catorce
transceptores. Las tres placas se conectan a la ZCU102 mediante FMC.
...
por completo en las pruebas posteriores. La traza completa de la sesión, desde el
descubrimiento de los transceptores hasta el último intercambio, es la siguiente:
...
comunicación serie en la sección~\ref{sec:pcb_slo}.
...
\begin{figure}[H]
    ...
    Obsérvese que la base de tiempos de las capturas de la derecha es cinco veces mayor que la de
    las de la izquierda: la transición pasa de unas pocas decenas de nanosegundos a varios
    centenares.}
    \label{fig:cdhs_slew}
\end{figure}
...
El barrido completo, tal y como salió por el terminal, es el siguiente. Se aprecian los dos
tramos de subida y bajada de la rampa aplicada a CH2, los dos escalones intermedios en los que la
fuente se dejó fija y la estabilidad de los otros tres canales durante todo el ensayo:
...
\begin{figure}[H]
    ...
    \caption{Señal PWM medida en el conector J5 de la placa CDHS: canal de 1~kHz con \textit{duty cycle} del 50\%.}
    \label{fig:cdhs_pwm}
\end{figure}
...
DE y RE cortocircuitadas; la V2 no llegó a fabricarse, de modo que todo lo que sigue mide la
revisión de cinco canales.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\textwidth]{IMG/aocs_soldada.jpg}
    \caption{Placas AOCS con componentes montados (vista superior de las dos unidades fabricadas).}
    \label{fig:aocs_foto}
\end{figure}
...
La traza completa de la sesión, con el descubrimiento de los cinco transceptores y los cuatro
pares de \textit{loopback} ejercitados, es la siguiente:
...
La medida también dejó al descubierto el error de conexión del eje Z descrito en la
sección~\ref{sec:diseno_aocs}: ese eje no entregaba señal en el conector porque las entradas de
