...
diferencia, la más pequeña, y así debe ser, porque apenas contiene los transceptores y un puñado
de registros. Ese ahorro de lógica no es gratuito: lo paga la CPU, como se ve más abajo.
...
kilobyte movido, acumulado sobre la sesión completa de pruebas. La tabla~\ref{tab:bench_irq}
recoge el desglose y la figura~\ref{fig:bench_irq} lo representa a escala.
...
comparación se recoge en la figura~\ref{fig:bench_latencia}.
...
primera columna de la tabla~\ref{tab:bench_throughput}, y la que aparece como línea de trazos en
la figura~\ref{fig:bench_throughput}.
...
barrido y la figura~\ref{fig:bench_baudios} los representa en escala logarítmica, donde la
diferencia de comportamiento entre ambas arquitecturas es inmediata.
...
son errores de transporte}, que son cero en las tres variantes: cuentan un byte cada vez que
...
mala: en la variante GPIO el propio cuello de botella de interrupciones frena la entrega, de modo
que el \textit{ring} se llena más despacio y desborda menos, mientras que el MCDMA entrega a la
