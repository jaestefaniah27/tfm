...
(C.6--C.8) recoge el detalle de implementación que los capítulos~\ref{cap:firmware}
y~\ref{cap:hardware} dejan fuera para no interrumpir el
...
hardware del capítulo~\ref{cap:resultados}.
...
comportamiento se resume en la sección~\ref{sec:normalizacion_fifo} del capítulo~\ref{cap:firmware}.
...
ZCU102, resumida en la sección~\ref{sec:testing_driver} del capítulo~\ref{cap:hardware}. Está repartida
...
\ref{sec:validacion_aocs} del capítulo~\ref{cap:resultados}.
...
capítulo~\ref{cap:resultados}. Se aprecian los dos tramos de subida y bajada de la rampa
