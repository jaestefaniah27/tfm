...
En este trabajo, unos binarios precompilados de RTEMS no habrían bastado. La
compilación desde código fuente fue la vía que permitió fijar con precisión la
arquitectura AArch64 objetivo, el BSP \texttt{zynqmp\_apu} concreto de la ZCU102 y la
versión~7 de RTEMS, y habilitar explícitamente el soporte SMP en la configuración del
núcleo. Sobre ese control total se apoyó después la integración del driver propio del
transceptor. Un binario precompilado genérico no habría garantizado esa combinación
exacta de arquitectura, BSP y opciones de núcleo.
...
ejemplo que interactuaba con los puertos GPIO conectados por AXI. Excitaba todas las
combinaciones posibles de la puerta AND en la PL y leía su salida. Fue necesario añadir
...
\texttt{0xA0000000}) y espera un instante a que la lógica de la PL se estabilice. Después lee la
...
direcciones de la PL. Sin él, el primer acceso a los GPIO aborta la ejecución.
...
de etapas hasta U-Boot. U-Boot, a su vez, carga el \texttt{rtems.img} en memoria y le cede el
control. Si el ensayo solo ejercita la PL, el \texttt{BOOT.bin} basta por sí solo, ya que lleva
dentro el \textit{bitstream}.
...
entradas valen uno. Esta traza fue la primera confirmación de que el flujo completo funcionaba
de extremo a extremo. Incluye el VHDL sintetizado en la PL, la imagen de arranque, la aplicación
de RTEMS y la comunicación PS$\leftrightarrow$PL por AXI. La salida se conserva en
...
compilación. Deduce el nombre de la aplicación del directorio desde el que se lanza y ejecuta
...
anterior para regenerar la imagen. A través de la conexión USB-serie con la placa, interrumpe
el \textit{autoboot}\footnote{Arranque automático que U-Boot lanza tras un breve tiempo de
espera; pulsando una tecla durante esa ventana se accede a su consola de comandos.} de U-Boot y
...
\texttt{loady}. Por último, lo escribe en la partición FAT\footnote{Partición de la tarjeta SD, con el
mismo formato que usa un USB corriente, de la que la ZCU102 lee los ficheros de arranque.} de
la SD mediante
\texttt{fatwrite}; \texttt{loady} y \texttt{fatwrite} son comandos de la consola de U-Boot.
Opcionalmente puede enviar también el \texttt{BOOT.bin}, pero no se utilizó porque el archivo es
grande y la conexión USB-serie es lenta. De esta manera, cada vez que se quería probar un cambio
en la aplicación de RTEMS, bastaba con ejecutar este único script y reiniciar la ZCU pulsando su
botón de \textit{ResetCPU}. Este botón reinicia el sistema de procesamiento sin cortar la
...
arrancar la placa. Una iteración completa lleva varios minutos. Cuando la placa no arranca, el
...
cambio en el VHDL. Limpiar y recompilar no da un resultado fiable. Por ello, el
...
El sistema no ofrece ninguna indicación del motivo. A ello se suma el tiempo de navegar por los
asistentes gráficos de cada herramienta. Para eliminar ambos costes se desarrolló el script
\texttt{generate\_boot.sh} (disponible en
\texttt{tfm/06\_firmware/boot\_scripts/} del repositorio del proyecto~\cite{repo_tfm}), un
\textit{wizard} interactivo en Bash. Encadena todos estos pasos de forma desatendida. El script
ofrece tres puntos de entrada, según el estado en que se encuentre el trabajo. Puede partir del
proyecto de Vivado y generar el \textit{bitstream} desde cero, partir de un proyecto con
\textit{bitstream} ya generado para exportar solo el XSA, o partir directamente de un XSA ya
exportado. A partir de ahí invoca Vitis en modo script mediante su API Python para crear la
plataforma y compilar el FSBL. Después llama a \texttt{bootgen} para ensamblar el BOOT.BIN final
con los componentes de arranque precompilados (PMUFW, BL31, U-Boot, DTB). El tiempo de iteración
hardware $\rightarrow$ imagen pasa así de varios minutos de interacción gráfica a una sola
ejecución de terminal. La imagen generada corresponde siempre al hardware recién sintetizado.
...
instancias y los nombres del bloque Zynq y del \textit{SmartConnect} a los que engancharse. La
llamada empleada en el diseño de catorce canales es
...
El diseño de las tres placas se realizó en \textbf{Altium Designer}. En los tres proyectos, antes
de colocar un solo componente, el primer paso fue \textbf{establecer las reglas de diseño}. Es un
paso que conviene no posponer: las reglas son el contrato que el comprobador de Altium verifica de
forma continua durante el rutado. Fijarlas al principio convierte las restricciones de fabricación
en un error detectado en el momento de cometerlo, en lugar de en un rechazo del fabricante cuando
el diseño ya está cerrado.
...
encarecen la tirada. Superarlos con margen no aporta nada. Diseñar directamente contra esas
...
controlada para las líneas RS-422/RS-485 y CAN, cuyo desequilibrio degradaría el rechazo a modo
común del que depende la inmunidad al ruido de un bus diferencial. La segunda es la
\textbf{longitud acotada} en las líneas de conmutación rápida procedentes del FMC.
...
Analog Devices, un transceptor RS-485/RS-422 de hasta 20~Mbps. Su pin de alimentación lógica
...
14 canales, declarado estáticamente en el array \texttt{rx\_lines[MAX\_TRANSCEIVERS]}. Esto evita
mezclar caracteres de UARTs distintas en la misma línea de consola.
