...
\textit{stride} de memoria entre instancias\footnote{Distancia en bytes entre la dirección base de una instancia y la de la siguiente; permite recorrer todos los transceptores sumando un desplazamiento fijo a la dirección de la primera.} y la dirección base de la primera de ellas. Más allá del
...
en VHDL}: se limita a propagarse hasta el pin \texttt{SLO} del chip transceptor de la PCB. El
driver solo expone el bit; su efecto eléctrico y la cuantificación de su impacto sobre la
velocidad máxima del enlace se tratan con la caracterización de la placa
(sección~\ref{sec:pcb_slo}).
