...
de modo que un .xsa nuevo no siempre se propaga a la imagen final y el sistema se comporta como
antes del cambio en el VHDL. Limpiar y recompilar no da un resultado fiable, por lo que el
...
receptor muestrea. La configuración representada es de 8 bits de datos, paridad par, un bit de
parada y orden LSB \textit{first}, con el dato \texttt{0x53}. El transmisor consume un \textit{tick}
por frontera de bit y pasa al doble de frecuencia en el campo de parada, donde cuenta semiperiodos.
El receptor, tras alinear su fase medio bit después del flanco de inicio, muestrea en el centro de
cada bit; el primer muestreo, en el centro del bit de inicio, es el que descarta los falsos inicios
de trama. Las zonas sombreadas \texttt{PreDE} y \texttt{PostDE} no están dibujadas a escala: duran
100~$\mu$s frente a los 8,7~$\mu$s del tiempo de bit a 115\,200~baudios.
...
\subsection{Mapa de registros de la interfaz hardware}
...
\subsection{Modelo de interrupciones con ISR maestra y tareas \textit{worker}}
...
\begin{enumerate}
    ...
    \item Para cada transceptor con la interrupción de recepción activa: la reconoce en el
    registro \texttt{INTC\_IAR}, deshabilita temporalmente la línea con \texttt{INTC\_CIE} y
    envía el evento
...
\begin{table}[H]
    \centering
    \small
    \begin{tabular}{@{}l p{0.40\textwidth}@{}}
        \hline
        \textbf{Función} & \textbf{Descripción} \\
        \hline
        \texttt{Transceiver\_Global\_INIT()} & Descubrimiento hardware e inicialización del INTC. \\
        \texttt{Transceiver\_Init(dev, id, cfg)} & Inicializa la instancia \texttt{dev} con el id y la configuración. \\
...
costa de limitar la velocidad máxima del enlace. En el transceptor empleado ese pin es un
habilitador de modo lento \textbf{activo a nivel bajo}, de modo que el nombre de la macro en
\texttt{transceiver.h} induce a error y el valor por defecto deja el limitador \textit{activado};
su efecto sobre la velocidad máxima se cuantifica en la sección~\ref{sec:resultados_pcb}.
...
Esta elección tiene un coste: las tarjetas diseñadas son \textbf{hardware de
