...
\begin{figure}[H]
    ...
    \caption{\textit{Block Design} del ejemplo de la puerta AND: bloque Zynq UltraScale+ MPSoC,
    los dos AXI GPIO y el módulo VHDL \texttt{AND\_GATE}.}
...
\begin{figure}[H]
    ...
    \caption{Generador de imágenes de arranque de Vitis, con los componentes que conforman el
    \texttt{BOOT.bin}.}
...
\textbf{Carga en la tarjeta SD y arranque de la placa.} \label{sec:carga_sd}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        \hline
    \end{tabular}
    \caption{Posiciones del selector de modo de arranque SW6 de la ZCU102. Adaptada
    de \cite{xilinx2023ug1182}.}
    \label{tab:sw6}
\end{table}
...
Opcionalmente puede enviar también el \texttt{BOOT.bin}, pero en la práctica no se utilizó porque este archivo resultaba ser demasiado grande y la conexión por USB-serial muy lenta. De esta manera, cada vez que se quería probar un cambio en la
...
Conviene señalar que poner en marcha todo este flujo costó bastante tiempo. Cada intento
implicaba sintetizar el diseño en Vivado, crear y compilar la plataforma en Vitis, crear y compilar
la aplicación, generar la imagen de arranque, compilar la imagen de RTEMS, copiar todo a la
tarjeta SD y arrancar la placa. Una iteración completa llevaba bastantes minutos y, cuando la
placa no arrancaba, no había ninguna indicación clara del motivo, así que había que volver atrás
y probar otra combinación de componentes o de parámetros. Solo después de varios intentos se
dio con la configuración que funciona.

Una vez conseguido el primer arranque correcto apareció un segundo problema: al modificar el
diseño VHDL y repetir el proceso, el sistema seguía comportándose como antes del cambio. La
causa era que Vitis reutiliza artefactos ya compilados de la plataforma y de la aplicación, de
modo que un .xsa nuevo no siempre se propagaba a la imagen final. Limpiar y recompilar no
resultó fiable, así que se optó por la solución directa: crear una plataforma y una aplicación
nuevas para cada cambio en la PL. Es más lento, pero garantiza que la imagen generada
corresponde realmente al hardware que se acaba de sintetizar.
...
FSBL y ensamblar el BOOT.BIN) resultaba tedioso y, sobre todo, propenso al error humano: basta
olvidar un componente, invertir su orden dentro del BIF o arrastrar un artefacto antiguo para que
la placa no arranque sin dar ninguna pista del motivo. A eso se sumaba el tiempo puramente
logístico de ir abriendo cada aplicación gráfica y navegar por sus asistentes. Para eliminar ambos
problemas se desarrolló el script \texttt{generate\_boot.sh} (disponible en
...
precompilados (PMUFW, BL31, U-Boot, DTB). Esto redujo el tiempo de iteración hardware
$\rightarrow$ imagen de varios minutos de clics a una sola ejecución de terminal, y garantiza que
la imagen generada corresponde siempre al hardware que se acaba de sintetizar.
...
\begin{itemize}
    ...
    de temporización de bit para las 54 velocidades soportadas, de modo que transmisor y
    receptor puedan trabajar a cualquiera de ellas.
...
Los apartados que siguen recorren cada uno de esos bloques en el mismo orden en que un dato los
atraviesa: primero la base de tiempos que comparten transmisor y receptor, después los dos caminos
de datos, y por último los bloques que los acoplan al PS. El
apartado~\ref{sec:bloque_canal} cierra el recorrido con el diagrama de conjunto
(figura~\ref{fig:transceptor_bloques}), una vez presentadas todas las piezas que aparecen en él.

\subsection{Oscilador controlado numéricamente (NCO)}

El transmisor y el receptor comparten una misma base de tiempos, que es el bloque que se describe
en primer lugar porque su salida, el \textit{tick}, es la referencia con la que ambos miden el
tiempo de bit.
...
Las dos entradas de control restantes son las que permiten a las máquinas de estados del
transmisor y del receptor anclar la fase del temporizador: \texttt{rst} pone a cero acumulador y
...
\texttt{tb\_NCO.vhd}, que barre las \textbf{29 velocidades del rango de uso} (de 9600 a
...
independientes; las entradas de la tabla por debajo de 9600~baudios y la de 4~Mbaudios no se
...
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize, caption={Salida de la simulación de \texttt{tb\_NCO.vhd} (extracto: primera, intermedia y última velocidad del barrido)}]
...
La figura~\ref{fig:cronograma_trama} recoge la secuencia completa de una trama sobre la línea, con
la relación temporal entre los tres elementos que gobiernan el enlace: la ventana de \texttt{DE},
los \textit{ticks} con los que el transmisor delimita cada bit y los instantes en los que el
receptor muestrea.
...
\begin{figure}[H]
    ...
    \fill[gray!12] (14.2,0.20) rectangle (16.0,4.90);

    % --- Líneas de separación de campo
    \foreach \x in {1.4,3.2,4.2,5.2,6.2,7.2,8.2,9.2,10.2,11.2,12.2,13.2,14.2,16.0}
    ...
    \node[fieldlbl] at (12.7,4.95) {P};
    \node[fieldlbl] at (13.7,4.95) {Parada};
    \node[fieldlbl] at (15.1,4.95) {\texttt{PostDE}};
    \node[fieldlbl] at (16.7,4.95) {Reposo};

    % --- DE
    \node[rowlbl] at (-0.15,4.30) {\texttt{DE}};
    \draw[wave] (0,4.0) -- (1.4,4.0) -- (1.4,4.6) -- (16.0,4.6) -- (16.0,4.0) -- (17.4,4.0);
    ...
    \draw[wave] (0,3.2) -- (3.2,3.2) -- (3.2,2.6) -- (4.2,2.6)
                ...
                -- (10.2,3.2) -- (11.2,3.2) -- (11.2,2.6) -- (13.2,2.6)
                -- (13.2,3.2) -- (17.4,3.2);
    ...
    \foreach \y in {4.6,3.2} {
        ...
        \fill[white] (14.98,\y-0.13) rectangle (15.32,\y+0.13);
        \draw[line width=0.6pt] (15.00,\y-0.16) -- (15.18,\y+0.16);
        \draw[line width=0.6pt] (15.14,\y-0.16) -- (15.32,\y+0.16);
    }
    ...
    \draw[wave] (0,1.4) -- (17.4,1.4);
    \foreach \x in {4.2,5.2,6.2,7.2,8.2,9.2,10.2,11.2,12.2,13.2,13.7,14.2}
    ...
    \foreach \x in {3.7,4.7,5.7,6.7,7.7,8.7,9.7,10.7,11.7,12.7,13.7} {
    ...
    \end{tikzpicture}}%
    \shorthandon{<>}
    \caption{Cronograma de una trama completa, con la configuración 8 bits de datos, paridad par,
    un bit de parada y orden LSB \textit{first} (dato \texttt{0x53}). El transmisor consume un
    \textit{tick} por frontera de bit y pasa al doble de frecuencia en el campo de parada, donde
    cuenta semiperiodos. El receptor, tras alinear su fase medio bit después del flanco de inicio,
    muestrea en el centro de cada bit; el primer muestreo, en el centro del bit de inicio, es el que
    descarta los falsos inicios de trama. Las zonas sombreadas \texttt{PreDE} y \texttt{PostDE} no
    están a escala: duran 100~$\mu$s frente a los 8,7~$\mu$s del tiempo de bit a
    115\,200~baudios.}
...
configuración son las mismas del transmisor y con idéntica codificación, de modo que ambos
...
\begin{sidewaysfigure}
    ...
    \caption{Diagrama de bloques de un canal del transceptor serie configurable, con los puertos
    de \texttt{CONFIGURABLE\_SERIAL} agrupados por función. El NCO proporciona la temporización de
    bit a transmisor y receptor; el receptor entrega los bits muestreados al registro de
    desplazamiento, que los normaliza y los escribe en la FIFO, de donde el PS los lee. La palabra
    de configuración es común a ambos sentidos, y \texttt{SLO} va del PS al pin del transceptor
    físico sin pasar por la lógica.}
...
\begin{table}[H]
    ...
    \begin{tabular}{llp{0.46\textwidth}}
        ...
        \hline
    \end{tabular}
    \caption{Reparto de bits de los vectores de interfaz del bloque
    \texttt{CONFIGURABLE\_SERIAL\_TOP}.}
...
de modo que el PS pueda ser avisado tanto de la llegada de datos como de la disponibilidad del
...
\begin{itemize}
    ...
    mismos genéricos, de modo que los canales puedan instanciarse y conectarse uno a uno en el
...
\texttt{tb\_CONFIGURABLE\_SERIAL\_TOP.vhd} conecta la salida \texttt{TX} del canal a su propia
...
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize, caption={Salida de la simulación de \texttt{tb\_CONFIGURABLE\_SERIAL\_TOP.vhd} (extracto: primeras y últimas configuraciones del barrido)}]
...
El driver que se describe a continuación es el de la \textbf{arquitectura de partida}, basada en
AXI GPIO. Funciona, y es la referencia contra la que se mide todo lo demás, pero durante la
validación con los catorce canales activos reveló un límite estructural: el procesador tiene
que tocar cada byte. Este límite motivó el rediseño completo del transporte entre el PS y la PL. Ese
trabajo, con las tres arquitecturas implementadas y su comparativa, se aborda en la
...
La interfaz expuesta al código de aplicación se reduce a las cinco funciones de la
tabla~\ref{tab:api}:
...
\textbf{Modo SLO.} El bit \texttt{BIT\_SLO} del registro de control \textbf{no actúa sobre la lógica
en VHDL}: se limita a propagarse hasta el pin \texttt{SLO} del chip transceptor de la PCB, que es
quien realmente limita la velocidad de flanco de sus salidas diferenciales. Un flanco más lento
reduce el contenido espectral de alta frecuencia y mejora la compatibilidad electromagnética, a
costa de limitar la velocidad máxima del enlace. Conviene advertir desde ya de una trampa que se
destapó al caracterizar la placa (sección~\ref{sec:resultados_pcb}): en el transceptor empleado el
pin es un habilitador de modo lento \textbf{activo a nivel bajo}, de modo que el nombre de la macro
en \texttt{transceiver.h} induce a error y el valor por defecto deja el limitador \textit{activado}.
...
AOCS, para que Sener pudiera validar su firmware sobre la ZCU102. Los esquemáticos (PDF)
y las BOM de las tres placas están disponibles en el repositorio del proyecto \cite{repo_tfm} bajo
\texttt{tfm/00\_docs/pcbs/}.

\subsection{Selección de componentes: la restricción de 1,8 V} \label{sec:seleccion_componentes}

Antes de describir cada placa conviene exponer la restricción que condicionó la selección de
componentes de las tres, porque no es una decisión de conveniencia sino un límite impuesto por la
plataforma.
...
componente que se conecte directamente a esos pines debe, por tanto, admitir una tensión de
alimentación de E/S (\textit{VIO}) de 1,8~V. La alternativa, interponer adaptadores de nivel entre
...
demostrado, por ser la opción preferible desde el punto de vista de la fiabilidad. La búsqueda
resultó infructuosa. Los transceptores de bus con herencia de vuelo disponibles en catálogo operan
...
y el catálogo de componentes cualificados para espacio resultaron, sencillamente, incompatibles.

La consecuencia es una decisión de diseño coherente con la filosofía NewSpace descrita en la
introducción: se recurrió a \textbf{componentes comerciales}, seleccionando dentro de ellos el
grado de calidad más alto disponible que cumpliera la restricción eléctrica. En la práctica, eso
significó priorizar piezas de \textbf{grado automotriz} (cualificación AEC-Q100, identificable por
el sufijo \texttt{Q1} en la referencia), que imponen rangos de temperatura extendidos y niveles de
...
Esta elección tiene un coste que conviene declarar: las tarjetas diseñadas son \textbf{hardware de
validación funcional, no hardware de vuelo}. Permiten verificar el subsistema de comunicaciones en
condiciones eléctricas realistas, que es su propósito, pero un modelo de vuelo exigiría rehacer la
selección de componentes atendiendo a tolerancia a radiación, \textit{outgassing} y cualificación
térmica, y muy probablemente replantear el nivel lógico de la interfaz para poder acceder al
catálogo cualificado.

\subsection{Reglas de diseño y proceso de fabricación} \label{sec:reglas_diseno}
...
Altium verifica de forma continua durante el enrutado, de modo que fijarlas al principio convierte
...
La figura~\ref{fig:placa_serial_top} muestra cómo se reparten ambos grupos sobre la placa
fabricada, a uno y otro lado del conector FMC.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{IMG/serial_pcb_top.jpg}
    \caption{Arquitectura general de la placa de comunicación serie: los siete drivers RS422
    (Driver7--Driver13, arriba), el conector FMC (P2, izquierda) y los siete drivers RS485
    (Driver0--Driver6, abajo).}
...
\begin{figure}[H]
    ...
    \caption{Hoja de nivel superior del esquemático de la placa CDHS: el bloque de conectores
    D-Sub-9 (J1--J6) a la izquierda, los bloques funcionales en el centro (CAN en J1, los tres
    canales RS en J2--J4, el adaptador de nivel de las PWM en J5 y el ADC de termistores en J6) y
    el conector FMC hacia el MPSoC a la derecha.}
    \label{fig:cdhs_top}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{Cara de conectores: los seis D-Sub-9 (J1--J6), los \textit{jumpers} de
        terminación del CAN y los de configuración H/F, SLR y terminación de los tres canales
        RS, con el módulo DC/DC PS1 en el borde izquierdo.}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{IMG/pcbs/cdhs_3d_bot.png}
        \caption{Cara de componentes: los tres transceptores RS422/RS485, los dos del bus CAN,
        el adaptador de nivel de las PWM, el ADC de termistores y el conector FMC (P7).}
    \end{subfigure}
    \caption{Render 3D de la placa CDHS, con la serigrafía de bloques que identifica cada
    subsistema sobre el conector por el que sale.}
    \label{fig:cdhs_3d}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Subsistema CAN de la placa CDHS: los dos transceptores TCAN1044AVDRQ1 (nominal y
    redundante), los diodos de protección ESDCAN24-2BLY, la terminación conmutable por
    \textit{jumper} y los condensadores de desacoplo.}
    \label{fig:cdhs_can}
\end{figure}
...
transceptor \textbf{THVD1424RGTR}. Este integrado llegó como sugerencia de un compañero del
proyecto LINCE en una reunión de seguimiento, posterior al diseño de la placa de comunicación
serie propia, y resultó ser una opción mejor que el LTC2865 empleado en aquella: además de
...
\begin{figure}[H]
    ...
    \caption{Canal RS de la placa CDHS con THVD1424RGTR: el cortocircuito DE--RE en el lado
    lógico, los diodos TVS SM712-02HTG sobre las cuatro líneas diferenciales y los cuatro
    \textit{jumpers} de configuración (H/F en P3, SLR en P4 y las terminaciones internas en
    P5 y P6).}
    \label{fig:cdhs_rs}
\end{figure}
...
cerrada del esquemático. Conviene declararlo desde el principio para poder leer los resultados
sin ambigüedad: las dos unidades fabricadas y toda la validación
...
\begin{figure}[H]
    ...
    \caption{Hoja de nivel superior del esquemático de la placa AOCS: seis canales RS
    repartidos entre cinco conectores (J1, J3, J5 y J8 con un canal cada uno, y J4 con dos en
    la hoja \texttt{RSx2}), el bloque de PWM de motores en J2 y los dos enlaces SpaceWire en
    J6 y J7, todos ellos entre el bloque de conectores D-Sub-9 y el conector FMC.}
    \label{fig:aocs_top}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{Cara de conectores: los ocho D-Sub-9, los \textit{jumpers} de configuración
        de cada canal RS, los puntos de prueba de las líneas lógicas y los dos bornes banana
        de alimentación de los motores (J9 y J10).}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{IMG/pcbs/aocs_3d_bot.png}
        \caption{Cara de componentes: los transceptores THVD1424RGTR de los canales RS, los
        repetidores LVDS de SpaceWire, la etapa de motores y el conector FMC (P1).}
    \end{subfigure}
    \caption{Render 3D de la revisión V2 de la placa AOCS. La serigrafía identifica cada
    bloque sobre el conector por el que sale, y los puntos de prueba quedan accesibles junto a
    los \textit{jumpers} en la cara de conectores.}
    \label{fig:aocs_3d}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Subsistema SpaceWire de la placa AOCS: cada enlace atraviesa dos repetidores LVDS
    DS90CP22M, uno por sentido de la comunicación (U7 y U9 para SPW1, U8 y U10 para SPW2). Cada
    enlace lleva sus cuatro pares diferenciales (DIN, SIN, SOUT y DOUT) protegidos con diodos
    TVS.}
    \label{fig:aocs_spw}
\end{figure}
...
Es un fallo característico de esta clase de tarjetas y conviene señalar por qué no lo detecta
ninguna comprobación automática: el diseño es eléctricamente correcto, no incumple ninguna
...
\begin{figure}[H]
    ...
    \caption{Placa CDHS tras el proceso de soldadura, con las dos caras a la vista: arriba la
    cara de conectores D-Sub-9 y \textit{jumpers} de configuración, y abajo la cara de
    componentes con los transceptores, el adaptador de nivel, el ADC y el conector FMC.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{IMG/arnes_can.jpg}
    \caption{Arnés de interconexión CAN nominal--redundante sobre el conector J1.}
    \label{fig:arnes_can}
\end{figure}
