...
arranque (BOOT.bin). Ambas estaban instaladas previamente en la máquina virtual.
...
Para validar su funcionamiento se desarrolló un \textit{testbench} dedicado que recorre las
velocidades soportadas y mide la frecuencia real del \textit{tick} generado. De esa campaña
procede la tabla del Anexo~1, y su lectura valida el diseño: el peor error de todo el rango es de
120~ppm (a 3,6864~Mbaudios), dos órdenes de magnitud por debajo de la tolerancia habitual del
$\pm$2\,\% de un receptor UART, y en la mayoría de las velocidades estándar el error es nulo o
inferior a 1~ppm. El NCO queda así acreditado como fuente de temporización fiable para todo el
rango de operación del transceptor.
...
\begin{itemize}
    ...
    bits y se resetea el acumulador del NCO ---de modo que la fase del temporizador arranque desde
    cero justo en el bit de inicio, sin arrastrar el desfase acumulado de la trama anterior--- y se
    pasa a \texttt{StartBit}.
...
\textbf{Generador de paridad.} Una red de puertas XOR calcula la paridad de los bits activos ---una
expresión por cada anchura de palabra--- y un multiplexor final aplica el modo configurado: paridad
...
La medida no eliminó los caracteres corruptos ---la causa estaba en el receptor, y se aborda en el
...
retienen el error hasta que el PS lo reconoce con \texttt{ERROR\_OK}. El borrado es síncrono, dentro
de la rama de reloj del proceso: en una versión anterior estaba escrito como una rama alternativa
\textit{previa} al flanco de reloj, lo que impedía ejecutar la rama síncrona mientras
\texttt{ERROR\_OK} estuviese activo. Como el PS mantiene ese bit permanentemente a uno, el efecto
era que el contador de bits nunca avanzaba y el receptor no llegaba a completar ninguna palabra.
Conviene tener presente que, con \texttt{ERROR\_OK} fijo a uno, las banderas se borran en cada
ciclo de reloj; para poder leerlas, el software debe desactivar ese bit.
...
\textit{tready} del canal AXI-Stream. La latencia del detector de flanco ---dos etapas de
...
que tocar cada byte--- que motivó el rediseño completo del transporte entre el PS y la PL. Ese
...
FMC ---la restricción de la sección~\ref{sec:seleccion_componentes}---, y que expone además un
...
El mapa completo de señales entre esta placa y el conector FMC se encuentra en el Anexo~1 de
...
Anexo~1 de este documento. El esquemático (PDF) y la BOM están disponibles en
...
Anexo~1. El esquemático (PDF) y la BOM están disponibles en \texttt{tfm/00\_docs/pcbs/aocs/}.
