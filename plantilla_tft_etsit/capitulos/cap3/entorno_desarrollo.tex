...
funciones de la tabla~\ref{tab:api}. El driver oculta el reparto de memoria, la gestión de
interrupciones y la sincronización entre tareas: el código de usuario se limita a abrir el canal,
fijar su configuración de protocolo y transferir bytes.

\begin{table}[H]
    \centering
    \small
    \begin{tabular}{@{}l p{0.44\textwidth}@{}}
        \hline
        \textbf{Función} & \textbf{Descripción} \\
        \hline
        \texttt{Transceiver\_Global\_INIT()} & Descubre el hardware, prepara el INTC e instala la ISR maestra. Devuelve el número de canales. \\
        \texttt{Transceiver\_Init(dev, id, cfg)} & Inicializa el canal \texttt{id} con la configuración \texttt{cfg}. \\
        \texttt{Transceiver\_SetRxCallback(dev, cb, arg)} & Registra el \textit{callback} de recepción. \\
        \texttt{Transceiver\_Read(dev, buf, maxlen)} & Extrae hasta \texttt{maxlen} bytes del \textit{buffer} de recepción. \\
        \texttt{Transceiver\_Send(dev, data, len)} & Encola \texttt{len} bytes y arranca la transmisión. \\
        \texttt{Transceiver\_SendString(dev, s)} & Variante de la anterior para cadenas terminadas en nulo. \\
        \hline
    \end{tabular}
...
El uso sigue siempre el mismo guion. \texttt{Transceiver\_Global\_INIT()} se invoca una vez al
arranque: descubre cuántos canales trae el \textit{bitstream}, prepara el controlador de
interrupciones e instala la rutina de interrupción común. Después, por cada canal que vaya a
usarse, \texttt{Transceiver\_Init()} recibe una estructura \texttt{Transceiver\_Config\_t} con la
tasa de baudios, los bits de datos, la paridad, los bits de parada y el orden de bits, y deja el
canal listo para operar.

La transferencia es asíncrona en los dos sentidos. Para transmitir, la aplicación llama a
\texttt{Transceiver\_Send()}, o a \texttt{Transceiver\_SendString()} para cadenas terminadas en
nulo: la función encola los bytes y retorna de inmediato, sin esperar a que salgan por la línea.
Para recibir, la aplicación registra con \texttt{Transceiver\_SetRxCallback()} una función propia,
el \textit{callback}, que el driver llama cada vez que hay bytes nuevos disponibles; al recibir ese
aviso, la aplicación los retira con \texttt{Transceiver\_Read()}, que copia hasta \texttt{maxlen}
bytes ya recibidos y devuelve el control aunque no haya ninguno. El \textit{callback} es solo una
notificación: cuando se dispara, el driver ya ha sacado del hardware los bytes y los tiene a
disposición de la aplicación.

La figura~\ref{fig:driver_camino_datos} resume el recorrido interno de un byte en cada sentido; las
dos últimas subsecciones lo detallan.
...
\textit{stride} de memoria entre instancias\footnote{Distancia en bytes entre la dirección base de
una instancia y la de la siguiente; permite recorrer todos los transceptores sumando un
desplazamiento fijo a la dirección de la primera.} y la dirección base del primero. El controlador
