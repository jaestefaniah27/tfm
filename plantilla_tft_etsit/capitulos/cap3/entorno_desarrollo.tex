...
\subsection{Validación del flujo de desarrollo de extremo a extremo}
...
figura~\ref{fig:block_design_and}, donde se aprecia la estructura que se repetirá en todos los
diseños del trabajo: el bloque del PS, los periféricos de la PL y el interconector AXI que los une,
con el bloque VHDL propio colgando de uno de los dos AXI GPIO.
...
correctamente. La pestaña \textit{Address Editor} de Vivado lista las direcciones base con las que el PS accede
a los registros de cada periférico de la PL. El proyecto completo de este ejemplo, con el diseño y
el código de las dos mitades, está en el repositorio bajo \texttt{tfm/00\_docs/AND\_example}. Por último, se generó el \textit{BitStream} y se exportó el
hardware como .xsa.
...
La lista siguiente detalla cada componente con su modo y su destino. El nivel de detalle es
deliberado: el modo y el orden de los componentes dentro del fichero BIF son la parte del
procedimiento que la documentación de Xilinx no reúne en un solo sitio, y un error en cualquiera de
los dos impide el arranque sin dar un mensaje útil.
...
\begin{itemize}
    ...
    de comprobación que iguala a par o a impar el número de unos de la palabra, no un código de
...
De esa campaña procede la tabla~\ref{tab:nco} del Anexo~C, y su lectura valida el diseño: el peor
...
señales de escritura y lectura, se recoge en el apartado C.6 del Anexo~C.
...
\texttt{0x08} respecto de la dirección base de la instancia, de modo que toda la interacción con el
hardware se reduce a escribir y leer dos palabras de 32 bits.

El registro de escritura reúne el dato a transmitir, tres bits de control y los campos de
configuración de protocolo; el de lectura, el byte recibido y las banderas de estado. La
...
\texttt{Transceiver\_Init()}, y permanecen fijos mientras el canal está abierto: el driver guarda
...
cuyas dos entradas están cableadas en la PL a constantes que el script de generación calcula al
...
canal hasta los catorce, sin recompilar: basta con que el hardware declare cuántas instancias trae y
dónde empiezan.
...
banco de entrada/salida digital de un circuito integrado, independiente de la de su núcleo, y fija
los niveles lógicos que la pieza acepta y entrega en sus pines. La alternativa, interponer adaptadores de nivel entre
...
programa New Space como LINCE. La práctica habitual en este ámbito es otra: emplear componentes de
...
impone rangos de temperatura extendidos y niveles de robustez muy superiores a los de grado
comercial. Es el escalón al que se recurre en misiones de bajo coste cuando no hay una pieza de
catálogo con herencia de vuelo que cumpla los requisitos eléctricos. Los transceptores \texttt{TCAN1044AVDRQ1} del
...
al nivel nativo de la plataforma y adaptar la interfaz a piezas de 3,3~V o 5~V, ya sea cambiando la
tensión del carril \texttt{VADJ} o interponiendo adaptadores de nivel.
...
Altium verifica de forma continua durante el rutado, por lo que fijarlas al principio convierte
...
transportan estas placas: \textbf{rutado en par diferencial} con impedancia controlada para las
...
El mapa completo de señales entre esta placa y el conector FMC se encuentra en el Anexo~C de
...
Anexo~C de este documento. El esquemático (PDF) y la BOM están disponibles en
...
Anexo~C. Los esquemáticos (PDF) y la BOM están disponibles en
...
transceptor, el driver y las tres placas se validan desde el propio MPSoC, de modo que hizo falta
construir el software que los pone a trabajar y que hace observable lo que ocurre: aplicaciones
sobre RTEMS que configuran cada canal, generan tráfico y comprueban lo recibido, y el equipamiento
de laboratorio que cierra los enlaces sobre cable real.

Este apartado describe esas herramientas. Son las que producen los resultados del capítulo~4, y
