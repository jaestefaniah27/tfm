...
transceptor serie de la sección~\ref{sec:transceptor_pl} es idéntico en las tres; lo único que
cambia es el transporte y el driver \texttt{transceiver.c/h} que lo maneja.
...
Esta sección describe la construcción de cada una, con las cifras de coste que arroja, y cierra
con la comparación que lleva a elegir la última. La comparativa cuantitativa completa, con
\textit{throughput} y latencia, se recoge en el capítulo de resultados
...
Su virtud es la simplicidad: es la variante con menos lógica (\textbf{6\,434 LUT}, un
2,35\,\% del dispositivo, 460 LUT por canal), no necesita ningún mapeo especial
...
La variante opera en placa y en transmisión alcanza \textbf{3,8 interrupciones por kilobyte}, frente
a las 1\,024 de la variante GPIO en ese mismo sentido. El límite de la variante no está, por tanto, en el rendimiento
...
(\textit{Look-Up Table}), un 14,7\,\% del dispositivo, esto es \textbf{2\,873 LUT por canal}: más
de seis veces la huella por canal de la variante GPIO. Ese coste condiciona además el
endurecimiento futuro del diseño, porque una triplicación modular (TMR, \textit{Triple Modular
...
\begin{sidewaysfigure}
    \centering
    \includegraphics[width=0.97\textheight]{IMG/Desarrollo/diagrama_arquitectura_mcdma.pdf}
...
UART ha terminado de \textit{emitirlo} por el cobre. Para una línea RS485 con control de
...
\begin{table}[H]
    ...
    \begin{tabular}{lll}
        ...
        \hline
    \end{tabular}
    \caption{Estructura del descriptor de \textit{buffer} (BD) del MCDMA.}
    \label{tab:mcdma_bd}
\end{table}
...
\subsubsection{Coste de la solución multicanal}

Compartir un único motor entre los catorce canales deja la ocupación en \textbf{14\,191 LUT}, un
5,18\,\% del dispositivo, \textbf{1\,014 LUT por canal}, y eso incluyendo lógica propia que las
otras dos variantes no llevan: el puente de transmisión, las catorce FIFO de recepción con su
árbitro y el bloque de fin de trama.

En interrupciones bastan las \textbf{tres líneas} ya descritas, MM2S, S2MM y fin de trama, de las
ocho que ofrece \texttt{pl\_ps\_irq0}. La ISR de recepción sigue teniendo que escanear registros de
estado para saber qué canales han completado, pero los escanea sobre un solo motor, y quedan cinco
líneas libres para futuras ampliaciones del diseño.

El precio de compartir se paga en el sentido de transmisión: con un único canal MM2S, dos tareas
que quieran escribir a la vez se turnan tras el \textit{mutex} del driver, y por eso
\texttt{Transceiver\_Send} es bloqueante aquí y no en las otras dos variantes. Cuánto cuesta esa
serialización se mide en la sección~\ref{sec:resultados_benchmark}.

% ---------------------------------------------------------------------------
\subsection{Comparación y elección de la arquitectura final}
% ---------------------------------------------------------------------------

Con las tres arquitecturas construidas y medidas sobre la misma placa, la elección se decide en
dos ejes: el coste de CPU y el coste en la lógica programable. La tabla~\ref{tab:coste_variantes}
recoge las cifras de ambos, ya justificadas en los apartados anteriores.

\begin{table}[H]
    \centering
    \begin{tabular}{lrrrr}
        \hline
        \textbf{Variante} & \textbf{IRQ/KB} & \textbf{LUT} & \textbf{LUT/canal} & \textbf{Líneas IRQ} \\
        \hline
        A --- GPIO  & 1\,250 & 6\,434  & 460   & 1$^{*}$ \\
        B --- DMA14 & 3,8$^{\dagger}$ & 40\,217 & 2\,873 & 28 \\
        C --- MCDMA & \textbf{3} & 14\,191 & \textbf{1\,014} & \textbf{3} \\
        \hline
        \multicolumn{5}{l}{\footnotesize $^{*}$ Un AXI INTC reduce por OR las veintiocho fuentes a una sola línea.} \\
        \multicolumn{5}{l}{\footnotesize $^{\dagger}$ Solo transmisión: sin lazo de recepción, la cifra no es comparable.} \\
        \multicolumn{5}{l}{\footnotesize El PS ofrece ocho líneas en \texttt{pl\_ps\_irq0}. Ocupación sobre las 274\,080 LUT del XCZU9EG.} \\
    \end{tabular}
    \caption{Coste de las tres variantes de transporte.}
    \label{tab:coste_variantes}
\end{table}

En \textbf{coste de CPU} la variante GPIO queda descartada por sí sola: sus 1\,250 interrupciones
por kilobyte la mantienen en \textit{livelock} y su rendimiento no mejora al subir la velocidad de
línea. Las dos variantes con DMA resuelven ese problema por igual, de modo que la diferencia entre
ellas no está en lo que rinden sino en lo que cuestan.

Ahí la comparación es concluyente. Replicar catorce motores cuesta \textbf{2,8 veces} la lógica de
compartir uno solo para el mismo número de canales, y esa distancia se triplica en cuanto se
plantee el endurecimiento por TMR: la variante DMA14 llevaría cerca de la mitad del dispositivo,
mientras que la multicanal sigue siendo asumible. En interrupciones la diferencia no es de grado
sino cualitativa, porque veintiocho líneas contra las ocho disponibles obligan a reducirlas por OR
y a escanear catorce registros de estado en cada evento, mientras que tres líneas caben con
holgura.

La variante MCDMA se adopta por ello como \textbf{solución final}. A cambio hay que asumir la
serialización de la transmisión y diseñar y verificar lógica propia en lugar de instanciar IP de
terceros, que es a lo que se dedica el resto de esta sección.
...
\begin{lstlisting}[style=terminal, caption={Salida de la simulación de \texttt{tb\_bridge\_rx.vhd}}]
-- Reset liberado --
...
\begin{lstlisting}[style=terminal, caption={Salida de la simulación del sistema completo, \texttt{tb\_system\_e2e.vhd}}]
...
La emulación es fiel al comportamiento eléctrico de un bus RS485: en reposo la línea está a
...
\begin{lstlisting}[style=terminal, caption={Salida de la simulación de \texttt{tb\_bench\_loopback.v}}]
  ok  reposo rd = 1
...
ocho pruebas: autodiagnóstico del lazo (T0), latencia de un \textit{frame} (T1), \textit{throughput} de un
...
4~Mbaudios (T6) y recuperación ante \textit{frames} corruptos (T7). El cronómetro para siempre
...
Con el lazo dentro de la FPGA, la señal no atraviesa los transceptores RS485 ni el cableado, de
