...
completas}, resumidas en la tabla~\ref{tab:tres_variantes}, y se midieron las tres con el mismo
programa de pruebas. El transceptor serie de la sección anterior es idéntico en las tres; lo único
que cambia es el transporte y el driver \texttt{transceiver.c/h} que lo maneja.
...
\begin{figure}[H]
    \centering
    \figpendiente{IMG/Desarrollo/diagrama_arquitectura_mcdma.pdf}{\textwidth}{arquitectura
    ...
    \caption{Arquitectura de la variante MCDMA, que es la solución final del trabajo: un único
    canal de transmisión y catorce de recepción sobre el mismo motor multicanal, con el puente
    VHDL repartiendo y recogiendo de los catorce transceptores. Es también la vista de conjunto
    del sistema completo, del \textit{buffer} en la DDR al pin del conector FMC.}
    \label{fig:mcdma_bd}
\end{figure}
...
\begin{itemize}
    ...
    cierra el paquete con \texttt{TLAST} y, sobre todo, fija el
...
quedó ni un byte inyectado sin salir por el \textit{stream}, que es exactamente el síntoma del
bloqueo que se estaba persiguiendo.
...
Es la prueba que da confianza para bajar a placa: si el TEST-2 pasa, el demultiplexado por
\texttt{TDEST} funciona con dos canales hablando a la vez, que es la condición que en placa se
manifestaba como «todo el tráfico acaba en el canal cero».
...
nuevo. Es un buen recordatorio de que una metodología de pruebas solo es fiable si se valida
