...
El Proyecto LINCE (\textit{Línea de INdustrialización de Cargas de pago y plataformas Espaciales}) es
...
ha desarrollado y validado este trabajo. Es una tarjeta de evaluación de propósito general de Xilinx, hoy AMD (en adelante, AMD), para el prototipado de
...
La \textbf{PL} es el dominio del hardware a medida: una matriz reconfigurable FPGA
...
periféricos físicos a la PL. En la figura~\ref{fig:zcu102} están señalados, junto al resto de
elementos de la tarjeta.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{IMG/zcu102.png}
    \caption{Tarjeta de evaluación ZCU102~\cite{xilinx2023ug1182}.}
    \label{fig:zcu102}
\end{figure}

La figura~\ref{fig:mpsoc_zynq} recoge el reparto, con los puertos AXI entre PS y PL recuadrados:
son el camino que recorren los datos de los catorce enlaces serie y el objeto de la comparativa
del capítulo~\ref{cap:resultados}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{IMG/Desarrollo/diagrama_mpsoc_zynq.pdf}
    \caption{Reparto PS--PL del MPSoC y entorno de la ZCU102.}
    \label{fig:mpsoc_zynq}
\end{figure}

\section{Vivado y Vitis (AMD): PS y PL}

Estas dos herramientas son las que han producido todo lo desarrollado en este trabajo: el VHDL
que ocupa la PL y el código C que corre en el PS. AMD distribuye dos entornos para el desarrollo sobre el Zynq UltraScale+: Vivado para la PL y
Vitis para el PS.
...
soporte de la placa (BSP, \textit{Board Support Package}): el mapa de memoria y los drivers
...
al modo común (CMRR, \textit{Common Mode Rejection Ratio}). Se suele usar en \textit{half-duplex} sobre dos hilos, con transmisión y recepción
...
mismo par y solo uno transmite a la vez, de modo que hace falta controlar el acceso al bus y cederlo
tras cada transmisión. En RS422 cada par tiene un único transmisor, y por eso el enlace no necesita arbitraje.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{IMG/Desarrollo/diagrama_topologia_rs.pdf}
...
La codificación \textit{Data-Strobe} (DS) supera esta limitación empleando dos líneas
diferenciales: \textit{Data} transmite directamente el flujo de datos, mientras que
\textit{Strobe} conmuta únicamente cuando el bit a transmitir es idéntico al anterior
(permaneciendo constante si el dato cambia). De este modo, se garantiza exactamente una transición
por período de bit entre ambas líneas. En recepción, el reloj se recupera de forma inmediata
aplicando una operación XOR a ambas señales y el dato se muestrea directamente de la línea
\textit{Data}, lo que elimina la necesidad de un alineamiento estricto de reloj y aporta una
alta tolerancia al desfase (\textit{skew}) entre pares. La figura~\ref{fig:data_strobe} recoge el
mecanismo sobre un flujo de diez bits.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{IMG/Desarrollo/diagrama_data_strobe.pdf}
    \caption{Codificación \textit{Data-Strobe} y recuperación del reloj.}
    \label{fig:data_strobe}
\end{figure}

El estándar define enlaces punto a punto
\textit{full-duplex} asíncronos de hasta 400~Mbps, con una pila que incluye una capa de red para
encaminar paquetes a través de \textit{routers} SpaceWire~\cite{ecss50spw,parkes2012spw}.

\subsection{CAN}

La placa CDHS lleva un bus CAN redundante, y su validación es uno de los resultados del
capítulo~\ref{cap:resultados}. El bus CAN (\textit{Controller Area Network}) nació en automoción y se usa como estándar de facto
