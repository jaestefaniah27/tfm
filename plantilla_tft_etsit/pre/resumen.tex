...
y midieron tres arquitecturas completas sobre el mismo transceptor: la primera basada en AXI GPIO,
la segunda con un controlador de acceso directo a memoria (DMA) por transceptor y la tercera con un
único controlador multicanal (MCDMA) gobernado por un puente diseñado en VHDL. La comparativa demuestra que la
...
architectures were implemented and measured on the same transceiver: the first one based on AXI
GPIO, the second one with a direct memory access (DMA) controller per transceiver, and the third
one with a single multichannel controller (MCDMA) governed by a bridge designed in VHDL. The comparison shows that
