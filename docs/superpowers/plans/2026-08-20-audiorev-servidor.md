...
**Goal:** Construir el servidor y la aplicación web que sirven el audio del TFM al móvil, registran las revisiones ancladas a la frase y las devuelven al agente por git y por API.
...
- La sesión web y la API del agente usan credenciales distintas: cookie firmada para la persona, token *bearer* para el agente.
...
Tres modos, en este orden de prioridad: si `trust_proxy_user` está definido, se confía en esa cabecera y `/login` devuelve 404; si no, se exige la cookie de sesión; la API del agente usa siempre el token *bearer*, con independencia del modo.
...
Formato del fichero, pensado para que el agente lo lea sin ambigüedad:
...
### Task 10: API para el agente y detección de frases obsoletas
...
`docs/audiorev/APLICAR_REVISIONES.md` debe indicar, para una sesión del agente desde el móvil:
...
git commit -m "Exponer las revisiones al agente y detectar las frases obsoletas"
```
...
4. Desde el móvil, pídele al agente que aplique las revisiones pendientes siguiendo `docs/audiorev/APLICAR_REVISIONES.md`.
