...
**Autor:** Jorge A. Estefanía Hidalgo (con Claude Code)
...
a Claude Code para aplicarlas sobre el LaTeX.
...
2. Anotar una revisión anclada a la frase exacta que se está oyendo, con el mínimo
   de fricción posible.
3. Que Claude Code recoja esas revisiones y las aplique sobre el fichero y la línea
   correctos.
...
- Editar el LaTeX desde la web. Las notas describen qué cambiar; el cambio lo hace
  Claude Code sobre el repositorio.
...
Al aplicar una revisión, Claude Code **busca por `sentence_text`, no por
...
La API que consume Claude Code usa un token *bearer* distinto de la sesión web,
...
deploy key. Desde el móvil basta con pedirle a Claude Code que aplique las
...
**Cierre del bucle.** Cuando Claude Code hace push del `.tex` corregido, el
...
| F4 | Cierre de sesión, escritura en `revisiones/`, push y API de estado | Claude Code aplica una revisión anotada desde el móvil |
